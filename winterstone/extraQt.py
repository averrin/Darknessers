            ...
            for line in self.source.splitlines(True):
                ...
                if style:
                    while rx.indexIn(QString(str(line)), pos) != -1:
                        token = rx.cap(1)
                        pos = rx.indexIn(QString(str(line)), pos)

                        #                    if not (len(token)>1 and token==last):
                        if token in matches:
                            if type(matches[token]).__name__ != 'list':
                                tmp = matches[token]
                                matches[token] = [tmp]
                            matches[token].append(rx.indexIn(QString(str(line)), pos))
                        else:
