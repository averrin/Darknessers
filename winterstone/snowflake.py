...
def loadIcons(icondir, ext=['.png', '.jpg']):
    ...
    for fname in dirList:
        print(os.path.join(icondir, fname), os.path.isdir(os.path.join(icondir, fname)))
        if os.path.isdir(os.path.join(icondir, fname)):
            subdirList = os.listdir(os.path.join(icondir, fname))
            for fi in subdirList:
                if fi[:4] in ext:
                    icons['fname/' + fi[:-4]] = os.path.join(icondir, fname, fi)
        elif fname[:4] in ext:
